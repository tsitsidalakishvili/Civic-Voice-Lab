...
from crm.clients.deliberation import clear_aura_db, delib_api_delete, delib_api_get
...
def render_admin_page():
    ...
    st.markdown("### Clear Aura DB")
    st.warning("Danger zone: deletes all nodes and relationships in Aura DB.")
    confirm_text = st.text_input(
        "Type 'CLEAR AURA DB' to confirm",
        key="admin_clear_db_confirm",
    )
    if st.button("Clear Aura DB", type="primary", key="admin_clear_db_btn"):
        if confirm_text.strip() != "CLEAR AURA DB":
            st.error("Confirmation text does not match.")
        else:
            result = clear_aura_db(confirm_text, show_error=True) or {}
            if result.get("ok"):
                deleted_nodes = result.get("deleted_nodes", 0)
                deleted_rels = result.get("deleted_relationships", 0)
                st.success(
                    f"Database cleared. Deleted {deleted_nodes} nodes and {deleted_rels} relationships."
                )
                st.rerun()
            else:
                st.error("Could not clear Aura DB.")
