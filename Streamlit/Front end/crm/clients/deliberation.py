...
def clear_aura_db(confirm_text: str, show_error=True):
    payload = {"confirm": str(confirm_text or "").strip()}
    return _request_json("POST", "/crm/admin/clear-db", payload=payload, show_error=show_error)


def run_due_diligence_analysis(payload, show_error=True):
    return _request_json("POST", "/due-diligence/analyze", payload=payload, show_error=show_error)


def run_due_diligence_debate_prep(payload, show_error=True):
    return _request_json(
        "POST", "/due-diligence/debate-prep", payload=payload, show_error=show_error
    )


def list_due_diligence_reports(subject, limit=8, show_error=False):
    subject_value = str(subject or "").strip()
    params = f"?limit={int(limit)}"
    if subject_value:
        params += f"&subject={quote_plus(subject_value)}"
    return _request_json("GET", f"/due-diligence/reports{params}", show_error=show_error)
