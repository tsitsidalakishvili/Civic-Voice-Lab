...
Local base URL: `http://localhost:8010`. Protected calls use either `Authorization: Bearer <token>` or the configured API-key header (default `X-FS-API-Key`). Never put credentials in URLs, logs, screenshots, or repository files.
...
| --- | --- |
| `/platform` | Authentication and access-gate status/verification |
...
| `/due-diligence` | Cases, sources, imports, entities, resolution, paths, findings, publications and connectors |
