...
export function FollowTheMoneyWorkspace({ caseId, activeStage, reportId, subjectName = '', subjectType = 'Person' }) {
  ...
  return <div className="ftm-workspace">
    ...
    {activeStage === 'sources' ? <>
      <WorkspaceHeader eyebrow="Evidence" title="Sources & connectors" description="See what is already connected, then add or refresh evidence through approved backend connectors." aside={<div className="ftm-count"><strong>{workspace?.counts?.datasets || datasets.length}</strong><span>connected datasets</span></div>} />
      <div className="ftm-source-layout">
        <SourceTopology datasets={datasets} />
        <section className="dd-connector-library">
          <div className="dd-connector-library__header"><div><span className="dd-card-kicker">Add or refresh evidence</span><h3>Connectors</h3><p>Open only the source you need. Every import stays source-scoped and preserves provenance.</p></div><span className="ftm-status is-ready">Backend mediated</span></div>
    ...
    {activeStage === 'entities' ? <>
      <WorkspaceHeader eyebrow="Evidence" title="Entity inventory" description="Inspect normalized people, organizations, assets, addresses, jurisdictions, and their source identities." aside={<div className="ftm-count"><strong>{workspace?.counts?.entities || entities.length}</strong><span>entities</span></div>} />
    ...
    {activeStage === 'resolve' ? <>
      <WorkspaceHeader eyebrow="Evidence quality" title="Identity resolution" description="Compare source identities without destroying original records or provenance." aside={<div className="ftm-count"><strong>{candidates?.summary?.pending || 0}</strong><span>pending</span></div>} />
    ...
    {activeStage === 'follow-the-money' ? <>
      <WorkspaceHeader eyebrow="Investigation" title="Follow the money" description="Ask repeatable graph questions, inspect evidence paths, and preserve promising leads." aside={<IconNetwork size={32}/>} />
      <details className="ftm-query-panel">
        <summary><span><strong>Path query tools</strong><small>Trace ownership, relatives, contracts, shared addresses, or the shortest evidence path.</small></span><span>Configure query</span></summary>
    ...
    {activeStage === 'findings' ? <>
      <WorkspaceHeader eyebrow="Outcome" title="Findings and hypotheses" description="Accept, reject, or keep researching evidence-backed claims. Nothing becomes a finding automatically." aside={<div className="ftm-count"><strong>{findings?.summary?.accepted || 0}</strong><span>accepted</span></div>} />
    ...
    {activeStage === 'publish' ? <>
      <WorkspaceHeader eyebrow="Outcome" title="Publish a defensible report" description="Reports are generated from analyst-accepted findings, with sources, caveats, and provenance preserved." aside={<div className={`ftm-readiness ${workspace?.publishReady?'is-ready':''}`}>{workspace?.publishReady?<IconCheck size={18}/>:<IconAlertTriangle size={18}/>}<strong>{workspace?.publishReady?'Ready to publish':'Not ready'}</strong></div>} />
