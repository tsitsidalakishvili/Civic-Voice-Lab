...
export function FollowTheMoneyWorkspace({ caseId, activeStage, reportId }) {
  ...
  return <div className="ftm-workspace">
    ...
    {activeStage === 'sources' ? <>
      ...
      <div className="ftm-dataset-grid">{datasets.map((dataset) => <article className="ftm-dataset" key={dataset.datasetId || dataset.sourceId}><div className="ftm-dataset__top"><IconDatabase size={19} /><span className={`ftm-status is-${dataset.status}`}>{dataset.status}</span></div><h3>{dataset.name}</h3><p>{dataset.type || 'Dataset'} · {dataset.ingestionMode || 'registered'}</p><div className="ftm-meter"><span style={{width:pct(dataset.provenanceCompleteness)}} /></div><small>{pct(dataset.provenanceCompleteness)} provenance complete</small><dl><div><dt>Statements</dt><dd>{dataset.statementCount}</dd></div><div><dt>Evidence</dt><dd>{dataset.evidenceCount}</dd></div><div><dt>Errors</dt><dd>{dataset.errorCount}</dd></div></dl>{dataset.url ? <a href={dataset.url} target="_blank" rel="noreferrer">Open source <IconExternalLink size={13} /></a> : null}</article>)}</div>
    </> : null}
    ...
    {activeStage === 'entities' ? <>
      ...
      <div className="ftm-split"><div className="ftm-entity-list">{entities.map((entity)=><button key={entity.id} type="button" className={entityDossier?.entity?.id===entity.id?'is-active':''} onClick={()=>openEntity(entity.id)}><span className="ftm-entity-type">{entity.type}</span><strong>{entity.displayName}</strong><small>{entity.entityRole || 'Unclassified role'} · {entity.statementCount} statements</small><span className={`ftm-resolution is-${entity.resolutionState}`}>{entity.resolutionState}</span></button>)}</div><aside className="ftm-dossier">{actionLoading&&!entityDossier?<p>Loading dossier…</p>:entityDossier?<><span className="dd-card-kicker">{entityDossier.entity.type} · {entityDossier.entity.entityRole || 'Entity'}</span><h3>{entityDossier.entity.displayName}</h3><p>{entityDossier.provenance.statementCount} statements across {entityDossier.provenance.sourceIds.length} sources.</p>{entityDossier.statementGroups.map((group)=><details key={group.predicate}><summary>{pretty(group.predicate)} <span>{group.statements.length}</span></summary>{group.statements.slice(0,8).map(statement=><div className={`ftm-statement is-${statement.assertionKind}`} key={statement.statementId}><strong>{statement.object?.value || statement.object?.entityId || '—'}</strong><small>{pretty(statement.assertionKind)} · {pretty(statement.verificationStatus)} · {pct(statement.confidence)}</small></div>)}</details>)}</>:<div className="ftm-dossier__empty"><IconFileCheck size={28}/><h3>Select an entity</h3><p>Review its statements, identifiers, connections, and provenance.</p></div>}</aside></div>
    </> : null}
